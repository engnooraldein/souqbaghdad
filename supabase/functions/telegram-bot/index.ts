...
function getCoreLocationKeyword(loc: string) {
  ...
  if (s.includes('مأمون') || s.includes('مامون')) return 'مأمون';
  if (s.includes('بيان') || s.includes('البيان')) return 'بيان';
  if (s.includes('مشرق') || s.includes('المشرق')) return 'مشرق';
  if (s.includes('عراقية') || s.includes('عراقيه')) return 'عراقية';
  if (s.includes('اشور') || s.includes('آشور')) return 'اشور';
  if (s.includes('بغداد')) return 'بغداد';
  if (s.includes('صيدلة') || s.includes('صيدله')) return 'صيدلة';
  if (s.includes('طب')) return 'طب';
  if (s.includes('هندسة') || s.includes('هندسه')) return 'هندسة';
...
async function handleSmartTransportSearch(chatId: string | number, rawText: string, fromUser: any, supabase: any, isGroup = false, userMessageId?: number | string) {
  ...
  const IRAQI_AREAS = [
    // الرصافة
    'البنوك', 'الشعب', 'حي أور', 'حي اور', 'جميلة', 'الطالبية', 'طالبية', 'مدينة الصدر', 'الحبيبية', 'البلديات', 
    'الأمين', 'الامين', 'حي المعلمين', 'الغدير', 'المشتل', 'بغداد الجديدة', 'الوزيرية', 'القاهرة', 'حي تونس', 
    'الصليخ', 'صليخ', 'سبع ابكار', 'سبع أبكار', 'الأعظمية', 'الاعظمية', 'الكريعات', 'الراشدية', 'الكمالية', 
    'الفضل', 'الميدان', 'الشورجة', 'الشرجية', 'الكفاح', 'شارع فلسطين', 'شارع النضال', 'شارع المغرب', 'باب المعظم', 
    'باب الشرقي', 'زيونة', 'الكرادة', 'الكرادة داخل', 'الكرادة خارج', 'العرصات', 'المسبح', 'الجادرية', 'الزعفرانية', 
    'جسر ديالى', 'المدائن', 'حي النصر', 'حي البساتين', 'سبع قصور', 'حي سومر', 'حي طارق', 'العبيدي', 'المعامل',
    
    // الكرخ
    'المنصور', 'اليرموك', 'الداوودي', 'الإسكان', 'الاسكان', 'حي الجامعة', 'حي الخضراء', 'حي العدل', 'العامرية', 
    'الغزالية', 'السيدية', 'الدورة', 'حي الشرطة', 'الشرطة الرابعة', 'الشرطة الخامسة', 'حي العامل', 'حي الجهاد', 
    'البياع', 'ابو دشير', 'أبو دشير', 'حي الاعلام', 'حي الإعلام', 'حي التراث', 'سويب', 'الشعلة', 'الحرية', 
    'العطيفية', 'الكاظمية', 'الطوبجي', 'الوشاش', 'الحارثية', 'القادسية', 'حي دراغ', 'المأمون', 'حي السلام', 
    'حي الفرات', 'جنينة', 'كرادة مريم', 'الصالحية', 'العلاوي', 'الشواكة', 'سبع البور', 'المحمودية', 'الرشيد', 
    'الطارمية', 'التاجي', 'أبو غريب', 'ابو غريب', 'الكرخ', 'الرصافة',
    
    // الجامعات والكليات والمعالم
    'الرافدين', 'كلية الرافدين', 'جامعة الرافدين', 'المستنصرية', 'الجامعة المستنصرية', 'التكنولوجية', 
    'الجامعة التكنولوجية', 'جامعة بغداد', 'النهرين', 'جامعة النهرين', 'كلية الاسراء', 'كلية التراث', 
    'كلية الرشيد', 'كلية دجلة', 'جامعة دجلة', 'كلية الفراهيدي', 'جامعة الفراهيدي', 'كلية اوروك', 
    'جامعة اوروك', 'كلية المعارف', 'كلية المأمون', 'كلية البيان', 'جامعة اشور', 'جامعة آشور'
  ];
  ...
  if (!destination) {
    ...
    else if (norm.includes('اشور') || norm.includes('آشور')) destination = 'جامعة اشور';
    else if (norm.includes('مامون') || norm.includes('مأمون')) destination = 'كلية المأمون الجامعة';
    else if (norm.includes('بيان')) destination = 'جامعة البيان';
    else if (norm.includes('مشرق')) destination = 'جامعة المشرق';
    else if (norm.includes('عراقية') || norm.includes('عراقيه')) destination = 'الجامعة العراقية';
  ...
  if (isGroup) {
    ...
    const shortMarkup = {
      inline_keyboard: [
        ...
        [{ text: '🛑 لكيت خط خلاص / إيقاف التنبيهات', callback_data: 'stop_alert_user' }],
    ...
    const waitlistMsg = 
      `📝 <b>تم تسجيل وتثبيت طلبك بنجاح! ✨</b>\n\n` +
      `📍 <b>المسار المطلوب:</b> من <b>${finalOrigin}</b> ⬅️ إلى <b>${finalDest}</b>\n` +
      `👤 <b>صاحب الطلب:</b> ${fromName}\n\n` +
      `🔔 <i>حالياً لا يوجد سائق مسجل بهذا المسار، لكن <b>طلبك مسجل بنظام الرادار الذكي</b>، وأول ما يسجل أي سائق أو يتوفر خط مناسب راح نبلغك برسالة وتنبيه فوري بالخاص! 🌹</i>`;

    const markup = {
      inline_keyboard: [
        [{ text: '🛑 لكيت خط خلاص / إيقاف التنبيهات', callback_data: 'stop_alert_user' }],
        [{ text: '➕ نشر طلبك كراكب في القنوات 📢', callback_data: 'publish_transport' }],
        [{ text: '🚌 تصفح جميع الخطوط المتوفرة بالموقع', url: 'https://www.souqbaghdad.store/transport' }]
      ]
    };

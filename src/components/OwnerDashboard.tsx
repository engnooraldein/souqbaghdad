...
import { SocialControlCenter } from './SocialControlCenter';
...
export default function OwnerDashboard({ ads, products, transportAds, onDeleteAd, onDeleteProduct, onDeleteTransportAd, onClose, onDeleteProfile }: {
  ...
  const [tab, setTab] = useState<'overview'|'social'|'visitors'|'users'|'guests'|'content'|'broadcast'|'recovery'|'verification'|'reports'|'logs'|'changelog'|'settings'|'promo_codes'>('overview');
  ...
  const [itemToDelete, setItemToDelete] = useState<{ id: string | number; type: 'ad' | 'product' | 'transport' | 'profile' } | null>(null);

  const getDeleteDialogProps = () => {
    if (!itemToDelete) return { title: '', description: '', confirmText: '' };
    switch (itemToDelete.type) {
      case 'ad':
        return {
          title: "هل أنت متأكد من حذف الإعلان؟",
          description: "سيتم حذف هذا الإعلان بشكل نهائي ومستمر من المنصة، ولا يمكن استرجاعه لاحقاً.",
          confirmText: "تأكيد حذف الإعلان"
        };
      case 'product':
        return {
          title: "هل أنت متأكد من حذف المنتج؟",
          description: "سيتم إزالة هذا المنتج نهائياً من المتجر والمعرض العام والبيانات المرتبطة به.",
          confirmText: "تأكيد حذف المنتج"
        };
      case 'transport':
        return {
          title: "هل أنت متأكد من حذف خط النقل؟",
          description: "سيتم مسح هذا الإعلان الخاص بخط النقل والتوصيل نهائياً ولا يمكن الرجوع عنه.",
          confirmText: "تأكيد حذف الخط"
        };
      case 'profile':
        return {
          title: "هل أنت متأكد من حذف الحساب بالكامل؟",
          description: "تحذير: سيتم حذف هذا الحساب الشخصي بشكل نهائي من قاعدة البيانات مع كافة الإعلانات والمنتجات وخطوط النقل المسجلة باسمه.",
          confirmText: "نعم، حذف الحساب نهائياً"
        };
      default:
        return { title: "هل أنت متأكد؟", description: "سيتم حذف هذا العنصر نهائياً.", confirmText: "تأكيد الحذف" };
    }
  };
      ...
      <div className="container mx-auto px-4 max-w-5xl">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div><div className="flex items-center gap-2"><h1 className="text-2xl font-bold text-white">داشبورت المالك</h1><span className="px-2.5 py-0.5 bg-gradient-to-r from-amber-500/20 to-yellow-500/10 border border-amber-500/40 text-amber-400 text-xs font-bold rounded-lg flex items-center gap-1 shadow-sm">🚀 الإصدار v1.9.0</span></div><p className="text-amber-400 text-xs mt-0.5">أجمل إصدار - تحليلات شاملة وإدارة كاملة للموقع المنصة حية ومتصلة</p></div>
          </div>
          <div className="flex items-center gap-3">
      ...
      <ConfirmationDialog
        ...
        onConfirm={() => {
          ...
          if (type === 'ad') {
            ...
            onDeleteTransportAd(Number(id));
          } else if (type === 'profile') {
            if (onDeleteProfile) {
              onDeleteProfile(String(id));
            }
            setDbUsers(prev => prev.filter(usr => usr.id !== id));
          }

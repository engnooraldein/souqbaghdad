...
export default function App() {
  ...
  const [chatTarget, setChatTarget] = useState<{userId:string;adId?:string;adTitle?:string}|null>(null);

  const openChat = (userId: string, adId?: string, adTitle?: string) => {
    if (!user) { requireAuth(); return; }
    setChatTarget({ userId, adId, adTitle });
    setShowChat(true);
  };
  ...
  return (
    <div className={`min-h-screen pwa-outer-container transition-colors duration-300 ${isDarkMode ? 'dark bg-[black] text-white' : 'bg-slate-50 text-slate-900'}`}>
      ...
      <AnimatePresence>
        ...
        {showChat&&user&&<Suspense fallback={null}><ChatModal currentUserId={user.id} currentUserName={user.name||'مستخدم'} openWithUserId={chatTarget?.userId} openWithAdId={chatTarget?.adId} openWithAdTitle={chatTarget?.adTitle} onClose={()=>{setShowChat(false);setChatTarget(null);}} onUnreadChange={setChatUnreadCount}/></Suspense>}

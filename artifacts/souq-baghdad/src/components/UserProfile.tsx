            ...
            >
              <div className="bg-gray-800/50 rounded-3xl p-6 border border-gray-700/50">
                ...
                <div className="space-y-4">
                  ...
                  {/* وضع توفير البيانات */}
                  <div className="w-full flex items-center justify-between p-4 bg-gray-700/50 rounded-xl" dir="rtl">
                    <div className="text-right">
                      <span className="text-white font-medium block">وضع توفير البيانات (Data Saver)</span>
                      <span className="text-gray-400 text-xs block mt-0.5">تقليل جودة الصور وتحميلها فقط عند الطلب لتوفير باقة الإنترنت الخلوي</span>
                    </div>
                    <button 
                      onClick={() => {
                        ...
                        try {
                          localStorage.setItem('souqDataSaver', String(nextVal));
                          window.dispatchEvent(new CustomEvent('souq-datasaver-change'));
                        } catch (err) {
                          console.error(err);
                        }
                      }}
                      className={`w-12 h-6 flex items-center rounded-full p-1 cursor-pointer transition-colors ${dataSaver ? 'bg-amber-500' : 'bg-gray-600'}`}
                      aria-label="وضع توفير البيانات"
                    >
                      <div className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform duration-300 ${dataSaver ? 'translate-x-6' : 'translate-x-0'}`} />
                    </button>
                  </div>

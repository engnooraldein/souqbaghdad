...
import { StoreSettingsWizard } from './StoreSettingsWizard';
...
يرجى تزويدي بطريقة الدفع لحجز البرومو كود.`;
  ...
  const [ef, setEf] = useState({ name:user.name, phone:user.phone, location:user.location, bio:user.bio||'', email:user.email||'', username:(user as any).username||'' });
        ...
        {tab==='account'&&(
          <div className="space-y-4">
            ...
            <div className={`rounded-2xl p-5 border ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-slate-200 shadow-sm'}`}>
              ...
              <div className={`mt-4 pt-4 border-t ${isDarkMode ? 'border-gray-700' : 'border-slate-100'}`}>
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2.5">
                    <div className="relative">
                      <div className="w-9 h-9 rounded-xl bg-white flex items-center justify-center shadow-sm shrink-0 border border-gray-200">
                    ...
                    <div>
                      <div className="flex items-center gap-1.5">
                        ...
                        {user.email && !user.email.endsWith('@souqbaghdad.store') ? (
                          ...
                          <span className="px-2 py-0.5 bg-gray-700/50 text-gray-400 text-[10px] font-medium rounded-full">
                            غير مرتبط
                          </span>
                        )}
                      </div>

...
export function ProductDetailModal({ product, onClose, isFav, onFav, user, storedUsers = [], onAuthRequired, onSellerClick, onViewDurationLogged, onImageZoom, onViewsUpdated, onStartChat }:{
    ...
    <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 touch-none">
      ...
      <motion.div initial={{scale:0.95,opacity:0}} animate={{scale:1,opacity:1}}
        ...
        <div className="p-4 sm:p-5">
          ...
          <div className={`grid gap-2.5 mb-2.5 ${onStartChat && user && product?.postedBy && user.id !== product.postedBy ? 'grid-cols-3' : 'grid-cols-2'}`}>
            ...
            {onStartChat && user && product?.postedBy && user.id !== product.postedBy && (
              <motion.button
                whileHover={{scale:1.01}} whileTap={{scale:0.99}}
                onClick={() => { onStartChat(product.postedBy!, String(product.id), product.title); onClose(); }}

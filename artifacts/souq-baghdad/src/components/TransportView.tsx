                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-gray-150 dark:border-gray-900/60">
                      ...
                      <div className="flex gap-2">
                        ...
                        {(user?.role === 'owner' || user?.role === 'admin') && (
                          <motion.button
                            onClick={(e) => { e.stopPropagation(); setPublishItem(ad); }}
                            whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                            className="flex items-center gap-1.5 px-3.5 py-2.5 bg-blue-500/10 text-blue-400 border border-blue-500/20 font-black rounded-xl text-xs hover:bg-blue-500/20 transition-all"
                          >
                            <Share2 className="w-3.5 h-3.5" /> نشر
                          </motion.button>
                        )}
                      </div>
                    </div>

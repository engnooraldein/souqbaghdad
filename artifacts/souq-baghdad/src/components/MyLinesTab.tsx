...
export function MyLinesTab({ userId, lines, onUpdateStatus, onDelete }: {
  ...
  return (
    <div className="space-y-4">
      ...
      <ConfirmationDialog
        isOpen={deleteLineId !== null}
        onClose={() => setDeleteLineId(null)}
        onConfirm={() => {
          if (deleteLineId !== null) {
            onDelete(deleteLineId);
            setDeleteLineId(null);
          }
        }}
        title="هل أنت متأكد من حذف هذا الخط؟"
        description="سيتم حذف هذا الإعلان/خط النقل بشكل نهائي من حسابك ومن المنصة ولا يمكن التراجع عن هذا الإجراء."
        confirmText="حذف الخط نهائياً"
        cancelText="تراجع"
        variant="danger"
      />
    </div>

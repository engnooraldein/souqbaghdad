...
export function ImageLightboxModal({ src, title, images, initialIdx = 0, onClose }: { src: string; title: string; images?: string[]; initialIdx?: number; onClose: () => void }) {
  ...
  const generateWatermarkedCanvas = async (imgUrl: string, itemTitle: string): Promise<HTMLCanvasElement | null> => {
    try {
      ...
      // --- 1. OVERLAY WATERMARK ON IMAGE ITSELF ---
      ctx.save();
      ctx.globalAlpha = 0.08; // subtle and clean
      ctx.fillStyle = '#ffffff';
      const fontSize = Math.max(14, Math.round(w * 0.025));
      ctx.font = `bold ${fontSize}px Cairo, system-ui, sans-serif`;
      ...
      // Center watermark logo badge
      ctx.save();
      ctx.globalAlpha = 0.16; // semi-transparent
      ...
      ctx.font = `bold ${Math.round(centerWatermarkSize * 0.16)}px Cairo, sans-serif`;
      ...
      ctx.font = `bold ${subFontSize}px Cairo, system-ui, sans-serif`;
      ctx.fillText(itemTitle, margin * 2, h + bannerH * 0.4);
  ...
  const handleDownloadAll = async () => {
    try {
      ...
      for (let i = 0; i < totalCount; i++) {
        ...
        if (canvas) {
          const link = document.createElement('a');
          link.download = `souq-baghdad-${title.replace(/\s+/g, '-')}-${i + 1}.jpg`;
          link.href = canvas.toDataURL('image/jpeg', 0.92);
          document.body.appendChild(link);
          link.click();
          document.body.removeChild(link);
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <div className="w-full max-w-xl mx-auto flex flex-col items-center gap-4 pb-8 z-10 px-4">
        {downloadAllProgress !== null && (
          <div className="text-amber-400 text-xs font-bold animate-pulse mb-1">
        ...
        <div className="flex flex-col sm:flex-row gap-3.5 w-full justify-center">
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
            ...
            className="flex-1 py-3.5 px-6 bg-gradient-to-r from-amber-500 to-amber-600 disabled:from-gray-800 disabled:to-gray-800 disabled:text-gray-500 text-black font-black rounded-2xl text-xs sm:text-sm flex items-center justify-center gap-2 shadow-lg shadow-amber-500/10 border border-amber-400/20 transition-all">
            {downloading && downloadAllProgress === null ? <Loader2 className="w-4 h-4 animate-spin" /> : <X className="w-4 h-4 rotate-45" />}
            تحميل الصورة الحالية بالشعار 📥
          </motion.button>

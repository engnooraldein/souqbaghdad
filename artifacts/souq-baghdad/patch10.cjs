...
content = content.replace(
  `<button onClick={e=>{e.stopPropagation();onSellerClick?.(e as any);}} className="flex items-center gap-1.5 hover:opacity-80 transition-opacity relative">`,
...
content = content.replace(
  `<button onClick={e=>{e.stopPropagation();onSellerClick?.(e as any);}} className="flex items-center gap-1.5 hover:opacity-80 relative">`,
...
// 6. SellerPublicPage
content = content.replace(
  `favorites, onToggleFav, user, onAuthRequired, onDeleteProfile, onActionMenu }:{`,
  `favorites, onToggleFav, user, onAuthRequired, onDeleteProfile, onActionMenu }:{\n  sellerId: string; allAds: Ad[]; allProducts: Product[]; onBack: ()=>void; onSelectAd: (a:Ad)=>void; onSelectProduct: (p:Product)=>void; favorites: string[]; onToggleFav: (id:string)=>void; user: User|null; onAuthRequired: ()=>void; onDeleteProfile?: ()=>void; onActionMenu?: (e: any)=>void;\n}) {\n  const onlineStatuses = useOnlineStatuses();`
);

content = content.replace(
  `<img src={sellerUser?.avatar || 'https://images.unsplash.com/photo-1633332755192-727a05c4013d?w=100'} alt={sellerUser?.name || 'البائع'} className="w-full h-full object-cover"/>\n            {sellerUser?.isVerified && <div className="absolute -bottom-2 -right-2 w-8 h-8 bg-blue-500 rounded-full border-4 border-gray-900 flex items-center justify-center" title="حساب موثق"><CheckCircle className="w-4 h-4 text-white"/></div>}`,
...
console.log('App.tsx patched carefully and correctly!');
